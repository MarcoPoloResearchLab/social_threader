...
- `transformation gateway`: The browser adapter that sends a thread transformation request to the application API.
...
- `split`: Divide source text into platform-sized chunks. Approved forms: split, splits, split.
- `transform`: Apply one closed product edit to source text. Approved forms: transform, transforms, transformed.
