...
- Install browser binaries once per machine with `npx playwright install` (or `npx playwright install --with-deps chromium` on Linux environments that need system dependencies).
- Execute `npm test` to run the Happy DOM harness (`npm run test:headless`) followed by the Playwright browser suite (`npm run test:browser`).
...
- The repository includes `.github/workflows/ci-playwright.yml`, a GitHub Actions workflow configured for Chromium-only, headless Playwright runs.
- The workflow installs browser binaries with operating-system dependencies, runs the Playwright suite with CI-tuned timeouts, and uploads HTML reports, JUnit output, and traces for debugging failed runs.
